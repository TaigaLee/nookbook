...
router.get("/outfits", async (req, res, next) => {
  try {
    const outfits = await RatingPicture.find({ category: "outfit" });
    res.render("ratingPicture/outfits.ejs", {
      pictures: outfits
    });
  } catch (err) {
    next(err);
...
router.get("/islands", async (req, res, next) => {
  try {
    const islands = await RatingPicture.find({ category: "island" });
    res.render("ratingPicture/islands.ejs", {
      pictures: islands
    });
  } catch (err) {
    next(err);
...
router.get("/:id", async (req, res, next) => {
  try {
    const postToShow = await RatingPicture.findById(req.params.id).populate(
      "user"
    );
    const commentsToShow = await Comment.find({ post: postToShow }).populate(
      "user"
    );
